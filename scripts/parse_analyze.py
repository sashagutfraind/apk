...
focus_categories = {
    ...
    'AgeDec=AgeLEQ20':{'fmt':'r-',      'altName':' 0-20',      'round':[5], 'sorting':12},
...
def collectStatusStats(statusFileNames, dataDir, params):
    ...
    for bucket, bucket_data in buckets:
        ...
        for idx,cnt in enumerate(reports[bucket]['agent_counts']):
            if cnt < 0.25*max(reports[bucket]['agent_counts']):
                max_time_axis = idx
                break
        print max_time_axis
...
def plot_prevalences(report, outputDir, params):
    ...
    for bucket, bucket_data in buckets:
        ...
        if bucket not in focus_categories:
            continue
        if plotting_round != 0 and (set(plotting_round).isdisjoint(focus_categories[bucket].get('round', []))):
            continue
        Ys = timeSeries[col][::int(days_per_Taxis_tick)]
        Ts = start_year + np.arange(0, len(timeSeries[col])/365., days_per_Taxis_tick/float(365), dtype=float)
        label = bucket.split('=')[-1]
...
def reportBatchStats(dataFileNames, params):
    ...
    if float(total_errors)/len(dataFileNames) > 0.05:
...
#no longer used.  uses xmlwitch with default values dictionary
def writeJobFile_old(dataDir, sample, jobNum, timeNow):
#writes the job file. the output directory is indicated by jobNum
    ...
    with xml.sweep(runs="1"):
        for param in default_run_parameters:
            if param in sample:
                params_value = str(sample[param]['value'])
            elif param == 'output_directory':
                params_value = outputDir #overwrites the setting in the lhs file!
            else:
                params_value = default_run_parameters[param]['value']
            params_type  = default_run_parameters[param]['constant_type']            
            #type:"constant" constant_type:"number" value:"42"
            xml.parameter("", name=param, type="constant", constant_type=params_type, value=params_value)
