...
log using trials_VE60_mod_Aug29.txt, text replace

/*** Vaccine simulation trials: VE 60 with 2-week blood draw ***/
/*** Biweekly RNA testing: 22 months + 9 for chronicity ***/
...
import delimited "vaccine_experiment_49_Aug_29_2022_results\events_history_`x'.csv", clear
drop v1
renvars, lower
rename v# t#, renumber
gen id = 1000 + _n
...
gen infected_before_final = first_converted <=4
...
**# dx on first +6
gen dx_period = first_converted + 12
lab var dx_period "period to diagnose chronic status"

**# count infections in first 22 months 
egen hcv_convert = anymatch(hcvrna1-hcvrna44), v(1)
egen periods_observed = anycount(hcvrna1-hcvrna66), v(0 1)
egen dropout = anymatch(hcvrna*), v(8)
lab var dropout "Lost to followup"
lab var periods_observed "Number of periods observed"

gen late_end = hcv_convert==0 & periods_observed>48
gen early_end = dropout==0 & hcv_convert==0 & periods_observed<44
...
**# fixed glitch that did not count cases infected in first period
...
**# fixed to count as chronic if resolved late
...
disp ""
disp "****** compare conversion (incident infection) rates by study arm ******"
...
save VEsims500_VE60_mod_Aug29.dta, replace
