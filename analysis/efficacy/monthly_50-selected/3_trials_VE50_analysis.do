...
hist pfe, percent title("Distribution of eVE: model aVE 50%†") w(.05) xlab(0(.1).8) norm lc(gray) fc(gray*.5) lw(vthin) graphregion(c(white)) ysc(titlegap(2)) xsc(titlegap(2)) note("†with monthly testing", si(medium)) caption("(b)", si(large)) 
graph export VEplot_Fig3b.jpg, replace
...
log using "VE50_May22_stats", replace
...
log using "VE50_May22_meta-summary", replace
...
** common effect model
meta set log_irr_chronic log_lb_irr log_ub_irr, studylabel(trial) studysize(total_N) common civartolerance(.035)
meta sum, common eform nostudies 
meta sum, common transform(efficacy) nostudies 

** exposed
meta set log_irr_chronic_exp log_lb_irr_exp log_ub_irr_exp, studylabel(trial) studysize(total_N) common civartolerance(.035)
meta sum, common eform nostudies 
meta sum, common transform(efficacy) nostudies 

** random effects model
meta set log_irr_chronic log_lb_irr log_ub_irr, studylabel(trial) studysize(total_N) civartolerance(.035)
...
scatter pred_VE rr_convert, msize(small) yti("Predicted VE") graphregion(c(white)) ysc(titlegap(2)) xsc(titlegap(2)) xsize(6) xlab(.4(.2)2.2) ylab(0(.1).8) xli(.75 1.33, lc(cranberry)) yli(.3 .6, lc(midblue) lpattern(dash)) yli(.5, lc(midblue)) caption("(a)") ti("Effect of Trial Arm Balance on Predicted VE over 500 Simulated Trials", size(medlarge)) subti("Model aVE 50%") 
graph export Fig4a_predicted.jpg, replace


scatter pfe rr_convert, msize(small) yti("Estimated VE") graphregion(c(white)) ysc(titlegap(2)) xsc(titlegap(2)) xsize(6) xlab(.4(.2)2.2) ylab(0(.1).8) xli(.75 1.33, lc(cranberry)) yli(.3 .6, lc(midblue) lpattern(dash)) yli(.5, lc(midblue)) caption("(a)") ti("Effect of Trial Arm Balance on Estimated VE in 500 Simulated Trials", size(medlarge)) subti("Model aVE 50%") 
graph export Fig4a_estimated.jpg, replace



/*** merge in summary data generated in simulation ****/
merge 1:1 trial using trials_VE50_May22_summary_wide, nogen


log using "vactrials_VE50_22_may22", replace
