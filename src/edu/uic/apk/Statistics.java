...
public class Statistics {
	...
	private static int incidence_now = 0; //daily incidence. updated during fire_status_change
	private static int losses_now = 0;    //daily losses (attrition, retirement, etc). updated during fire_status_change
	...
	public Hashtable <String,Double> collect_stats() {
		...
		currentData.put("incidence", new Double(Statistics.incidence_now));
		currentData.put("losses",    new Double(Statistics.losses_now));
	...
	public static void fire_status_change(AgentMessage message, IDU agent, String message_info, HashMap details) {
		...
		switch(message) {
			case activated:
				fire_helper(time_now, eventClass, agent.hashCode(), message, message_info, "-", "-", "-", "-", agent.toString());
				break;
			case chronic:
				fire_helper(time_now, eventClass, agent.hashCode(), message, message_info, "-", "-", "-", "-", agent.toString());
				break;
			case deactivated:
				Statistics.losses_now += 1;
				fire_helper(time_now, eventClass, agent.hashCode(), message, message_info, "-", "-", "-", "-", agent.toString());
				break;
			case exposed:
				break; //not recorded - too many events
			case infected:
				Statistics.incidence_now += 1;
				boolean is_agents_first_exposure = (agent.getLastExposureDate().getYear() < 1900);
				if (is_agents_first_exposure) {
	...
	private static void fire_helper(double time_now, String eventClass, int agentID, AgentMessage message, Object message_info,
	...
	public void statsDailyAction(){
		...
		try{
			...
			incidence_now = 0;  //initialize accumulator for current tick
			losses_now    = 0;  //initialize accumulator for current tick
		} catch (Exception ex) {
